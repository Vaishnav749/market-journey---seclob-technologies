...
class Renewalpackagesignal extends StatefulWidget {
  const Renewalpackagesignal({super.key});

  @override
  State<Renewalpackagesignal> createState() => _RenewalpackagesignalState();
}

class _RenewalpackagesignalState extends State<Renewalpackagesignal> {
  var userid;
  List<dynamic> renewalsiPackage = [];
  List<dynamic> renewalcosiPackage = [];
  XFile? _imageFile;
  ...
  Future<void> _viewpak() async {
    try {
      ...
      var response = await homeservice.viewpackageaddon();
      log.i('Profile data show.... $response');
      setState(() {
        renewalsiPackage = response['addOns'] ?? [];
      });
    } catch (error) {
  ...
  Future<void> _viewrenewpak() async {
    try {
      ...
      log.i('Profile data show.... $response');
      setState(() {
        renewalcosiPackage = response['anotherPackages'] ?? [];
      });
    } catch (error) {
      log.e('Error fetching subscription history data: $error');
    }
        ...
        child: Column(
          children: [
            ...
            ListView.builder(
              ...
              itemCount: renewalsiPackage.length,
              itemBuilder: (BuildContext context, int index) {
                var transaction = renewalsiPackage[index];
                return Padding(
                  padding:
            ...
            ListView.builder(
              shrinkWrap: true,
              physics: NeverScrollableScrollPhysics(),
              itemCount: renewalcosiPackage.length,
              itemBuilder: (BuildContext context, int index) {
                var transaction2 = renewalcosiPackage[index];
                return Padding(
                  padding:
                  const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
                  child: GestureDetector(
